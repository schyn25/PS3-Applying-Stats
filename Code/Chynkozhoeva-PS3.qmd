...
```
The results suggest significant differences in body length among the five species of bats. Specifically, the hoary bat (LACI), silver-haired bat (LANO), little brown bat (MYLU), and eastern pipistelle (PESU) have significantly different body lengths compared to the big brown bat (EFPU).
...
We will look at this problem, firstly by looking at the relationship between the number of ticks found and sex (part A) and then, at the relationship between the number of ticks found and age of the bats (part B). Since the questions asks about association between categorical predictor and response variables, our answer could found by conducting Chi-squared test.

### Part A: Species vs sex

```{r}
tick_num <- bats |> group_by(Species, sex) |>
  summarise(total_ticks = sum(tick_num))
```

```{r}
ggplot(tick_num, aes(x = Species, y = total_ticks, fill = sex)) +
...
```{r}
sex_mat <- xtabs(total_ticks ~ Species + sex,
                  data = tick_num)
```
...
evidence of an association between total number of ticks and the number of ticks found on the bats is associated with their sex based on the small p-value () that we found (X-squared = 9.7767, df = 4, p-value = 0.04436)

### Part B: Specis vs age

```{r}
tick_num2 <- bats |> group_by(Species, age) |>
  summarise(total_ticks = sum(tick_num))
```

```{r}
ggplot(tick_num2, aes(x = Species, y = total_ticks, fill = age)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  theme_bw()
...
```{r}
age_mat <- xtabs(total_ticks ~ Species + age,
                 data = tick_num2)
```
